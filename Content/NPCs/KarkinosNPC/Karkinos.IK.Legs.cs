...
namespace AbyssOverhaul.Content.NPCs.KarkinosNPC
{
    public partial class Karkinos
    {
        ...
        public KarkinosLeg[] _KarkinosLegs = new KarkinosLeg[4];
        ...
        public void InitializeLegs()
        {
            if (_KarkinosLegs == null)
                _KarkinosLegs = new KarkinosLeg[4];

            for (int i = 0; i < MAX_LIMBS; i++)
            {
                float scale = i == 0 || i == 3 ? 4.8f : 4.4f;
                ...
                _KarkinosLegs[i] = new(skeleton, i);
            }
        }
        public sealed class KarkinosLeg
        {
            ...
            public float Phase;
            public KarkinosLeg(IKSkeleton skeleton, int index)
            {
                Skeleton = skeleton;
                Index = index;
            }
        ...
        public void KarkinosLegUpdate()
        {
            if (_KarkinosLegs is null)
                return;


            for (int i = 0; i < _KarkinosLegs.Length; i++)
            {
                ref var limb = ref _KarkinosLegs[i];
                bool isLeft = i < 2;
                float side = isLeft ? -1f : 1f;

                limb.Phase = i / (float)_KarkinosLegs.Length * 2;
                limb.Phase *= side;
                limb.Phase += NPC.whoAmI * 0.2f;
                float lateralSpacing = 12f * (i == 0 || i == 3 ? 1f : 0.7f);
                float stepThreshold = 32f * (i == 0 || i == 3 ? 1.1f : 0.7f);

                float stepSpeed = 0.09f;
                float stepHeight = 12f;
                ...
                for (int s = -6 + (int)side * 2; s <= 6 + (int)side * 2; s += 2)
                {
                    ...
                    Point? ray = LineAlgorithm.RaycastTo(start, end, debug: i == 0);
                    ...
                    for (int j = 0; j < _KarkinosLegs.Length; j++)
                    {
                        if (j == i)
                            continue;

                        if (_KarkinosLegs[j].IsStepping)
                            continue;

                        Vector2 other = _KarkinosLegs[j].Tip;
                        float d = Vector2.Distance(other, candidate);

                        const float PreferredSeparation = 12f;
                    ...
                    float score = -directionalScore + distancePenalty / 2 + reachBonus * Utilities.Convert01To010(s / 12f - moveDir.X) - RejectCadidatesBeneathBody - separationPenalty;
                ...
                _KarkinosLegs[i].Skeleton.Update(NPC.Center + ActualLimbOffsets[i], _KarkinosLegs[i].PlantLocation);
            }
        }

        void PlayStepEffect(ref KarkinosLeg limb)
        {
            SoundEngine.PlaySound(SoundID.Mech with { Pitch = -1, PitchVariance = 0.4f, Volume = 1f, MaxInstances = 0 }, (NPC.Center + limb.PlantLocation) / 2);
            foreach (var player in Main.ActivePlayers)
            {
                ...
                if (dist < maxRange)
                {
                    ...
                    {
                        ScreenShakeSystem.ShakeAt
                        (
                            limb.PlantLocation,
                            shakeMagnitude, 1,

                            dampingPower: 0.7f - strength * 0.01f
                        );
                    }
                }
            }
