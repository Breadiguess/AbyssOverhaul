...
namespace AbyssOverhaul.Content.NPCs.Critters.DepthFeeder
{
    public class DepthFeeder : ModNPC, IEcologyParticipant
    {
        ...
        public override void AI()
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient && !HasCreatedSchool && NPC.wet && !NPC.Ecology().SpawnedFromActor)
            {
            ...
            if (shouldTurnAround && (closestPredator is null || !NPC.WithinRange(closestPredator.Center, 400f)))
            {
                ...
                float distanceToTileOnRight = Utilities.DistanceToTileCollisionHit(NPC.Center, NPC.velocity.RotatedBy(MathHelper.PiOver2)) ?? 999f;
            ...
            // NPC.velocity = NPC.DirectionTo(Main.MouseWorld);
        }
        ...
        public void DoSchoolingMovement()
        {
            ...
            {
                float predatorAvoidanceInterpolant = Utils.GetLerpValue(400f, 250f, NPC.Distance(closestPredator.Center), true);
                predatorAvoidanceInterpolant *= float.Lerp(0.1f, 1f, closestPredator.Opacity);
                NPC.velocity += NPC.SafeDirectionTo(closestPredator.Center) * predatorAvoidanceInterpolant * -1.3f;
            }
